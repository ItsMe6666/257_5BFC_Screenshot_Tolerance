...
for filename in image_list:
    ...
    img = img.point(lambda p: 0 if 0 <= p <= 150 else 255)
    #img.show()
    # 辨識圖片中的文本(英文和數字)
    ...
    for number in numbers:
        if len(number) == 3:
            if len(numberlist) == 0:
                ...
                if avg - 10 <= int(number) <= avg + 10:#濾掉奇怪的數字
                    first_three_numbers = int(number)
                    numberlist.append(first_three_numbers)
                    break
    print("照片學號為:" + number)        
    # 將文件名改為對應數字
    new_file_name = str(first_three_numbers) + '.png'
